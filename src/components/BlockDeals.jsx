...
export default function BlockDeals() {
  ...
  const fetchBlockDeals = async () => {
    ...
    try {
      ...
      const result = await nseDataAPI.getBlockDeals(
        formatForAPI(from),
        formatForAPI(to)
      );
      ...
      if (result.status === 'success') {
        ...
        setError('Failed to fetch block deals');
      }
    } catch (err) {
      setError(err.message || 'Error fetching block deals');
      console.error('Block deals error:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-[95%] xl:max-w-[1600px] mx-auto">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900">Block Deals</h3>
                <p className="text-sm text-gray-600">Large block transactions on NSE</p>
              </div>
            </div>
            <button
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">Loading block deals...</p>
          </div>
        )}
          ...
          <>
            {data.length === 0 ? (
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                <p className="text-gray-600">No block deals found for the selected period.</p>
              </div>
            ) : (
                      ...
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="font-medium text-gray-900">
                            {formatCurrency(deal.VALUE || deal.value || deal.TRADE_VALUE || deal.trade_value)}
                          </span>
                        </td>
